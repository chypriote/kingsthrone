...
import { FHero, FHeroStats, FShop, TourneyFight, TourneyRewardItem } from '~/types/tourney'
import { DMOngoingFight } from '~/types/goat/DMTourney'
...
const buyShop = async (shop: FShop[]): Promise<void> => {
	...
	for (const item of items) {
		...
		if (buyable) {
			...
			state.status.updateFromFight((await goat.dmBuyTourneyBoost(item)).fight)
			return
		}
	}
}
...
const doFight = async (status: DMOngoingFight) => {
	...
	const battle = await goat.dmFightHero(op)
	...
	if (battle.win.fight.winnum && battle.win.fight.winnum % 3 === 0) {
		const reward = (await goat.dmGetReward()).items[0]
...
const prepareFight = async (opponent: string|null, hid: number|null): Promise<DMOngoingFight> => {
	const status = await goat.dmGetTourneyInfos()
	...
	if (opponent) {
		...
		return await goat.dmChallengeOpponent(opponent, hero?.id || 0)
	}
	...
	if (state === 3) {
		...
		return await goat.dmStartTokenTourneyFight()
	}
	...
	return await goat.dmStartTourneyFight()
}
...
export const doDeathmatch = async (account: string, opponent: string|null = null, hid: string|null = null): Promise<void> => {
...
doDeathmatch(process.argv[2], process.argv[3], process.argv[4]).then(() => {process.exit()})
