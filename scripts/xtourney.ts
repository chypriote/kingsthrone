...
const buyShop = async (shop: FShop[]): Promise<void> => {
	...
	for (const item of items) {
		...
		if (buyable) {
			...
			state.status.updateFromFight((await goat.xsBuyTourneyBoost(item)).fight)
			return
		}
	}
}
...
const doFight = async (status: OngoingFight) => {
	...
	const battle = await goat.xsFightHero(op)
	...
	if (battle.win.fight.winnum && battle.win.fight.winnum % 3 === 0) {
		const reward = (await goat.xsGetReward()).items[0]
...
const prepareFight = async (opponent: string|null, hid: number|null): Promise<OngoingFight> => {
	const status = await goat.xsGetTourneyInfos()
	...
	if (opponent) {
		...
		return await goat.xsChallengeOpponent(opponent, hero?.id || 0)
	}
	...
	if (state === 3) {
		...
		return await goat.xsStartTokenTourneyFight()
	}
	...
	return await goat.xsStartTourneyFight()
}
